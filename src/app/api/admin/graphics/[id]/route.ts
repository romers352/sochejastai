...
export async function DELETE(req: Request, context: any) {
  ...
  try {
    const [rows] = await pool.execute("SELECT src FROM graphics WHERE id = ?", [idNum]);
    const row = Array.isArray(rows) && rows.length ? (rows as any)[0] as { src: string } : null;
    const src: string | null = row?.src || null;
    if (src && src.startsWith("/uploads/graphics/")) {
    ...
    await pool.execute("DELETE FROM graphics WHERE id = ?", [idNum]);
