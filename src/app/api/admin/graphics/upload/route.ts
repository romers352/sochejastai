...
export async function POST(req: Request) {
  ...
  try {
    ...
    await pool.execute(
      "CREATE TABLE IF NOT EXISTS graphics (id INT UNSIGNED NOT NULL AUTO_INCREMENT, title VARCHAR(255) NOT NULL, src VARCHAR(1024) NOT NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (id), INDEX (created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
    );
    const [result] = await pool.execute(
      "INSERT INTO graphics (title, src) VALUES (?, ?)",
      [title, publicSrc]
    );
    const id = (result as any).insertId;
    return NextResponse.json({ ok: true, id, src: publicSrc });
  } catch (e: any) {
    const message = e?.message ? String(e.message) : "Failed to upload graphic";
